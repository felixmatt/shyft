...
class CalendarTestCase(unittest.TestCase):
    """Verify and illustrate the Calendar & utctime from the api core, using pyunit.
    Note that the Calendar not yet support local/dst semantics (but plan to do so)
    Nevertheless, keeping it here allow users of api-Core to use start practicing
    utctime/calendar perimeter.
    ...
    def test_utctime_now(self):
        ...
        b= self.utc.time(api.YMDhms(x.year,x.month,x.day,x.hour,x.minute,x.second))
