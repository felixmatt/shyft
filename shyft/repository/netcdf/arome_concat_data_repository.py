...
class AromeConcatDataRepository(interfaces.GeoTsRepository):
    ...
    def __init__(self, epsg, filename, nb_fc_to_drop=None, nb_fc_interval_to_concat=None, selection_criteria=None):
    ...
    def _validate_selection_criteria(self):
        ...
        if list(s_c)[0] == 'unique_id':
            ...
            raise AromeConcatDataRepositoryError("Selection using polygon not supported yet.")
        elif list(s_c)[0] == 'bbox':
            if not (isinstance(s_c['bbox'], list) and len(s_c['bbox']) == 2):
                raise AromeConcatDataRepositoryError("bbox selection criteria should be a list with two lists.")
    ...
    def bounding_box(self):
        ...
        if self._bounding_box is None:
            raise AromeConcatDataRepositoryError("A bounding box must be provided.")
        ...
        if(list(self.selection_criteria)[0]=='bbox'):
            ...
            if sum(x_upper == x_lower) < 2:
                if sum(x_lower) == 0 and sum(x_upper) == len(x_upper):
                    raise AromeConcatDataRepositoryError("Bounding box longitudes don't intersect with dataset.")
            ...
            if sum(y_upper == y_lower) < 2:
                if sum(y_lower) == 0 and sum(y_upper) == len(y_upper):
                    raise AromeConcatDataRepositoryError("Bounding box latitudes don't intersect with dataset.")
    ...
    def _get_data_from_dataset(self, dataset, input_source_types, fc_selection_criteria_v,
        ...
        #    raise AromeConcatDataRepositoryError("Could not find all data fields")
